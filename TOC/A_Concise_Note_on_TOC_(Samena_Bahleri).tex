...
\begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto]
    ...
    \path[->]
        (w0) edge node[above] {$a$} (w1)
        (w1) edge node[above] {$b$} (w2)
        (w2) edge node[above] {$b$} (w3)
        (w3) edge[left=20] node[right] {$a,b$} (w4)
        (w4) edge[loop below] node {$a,b$} ()
        (w0) edge[loop above] node {$b$} ()
        (w1) edge[left=30] node[left] {$a$} (w4);
\end{tikzpicture}
}
\end{center}
...
\begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto]
    ...
    \path[->]
        (w0) edge node[above] {$a$} (w1)
        (w1) edge[loop above] node {$a$} ()
        (w0) edge[loop below] node {$b$} ()
        (w1) edge[bend left] node[below] {$b$} (w0);
\end{tikzpicture}
}
\end{center}
...
\begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto]
    ...
    \path[->]
        (w0) edge[loop above] node {$1$} (w0)
        (w0) edge[bend left] node[above] {$0$} (w1)
...
\[
L(M_1) = L(M_2) =
\{w \in \{0,1\}^* \mid \text{number of } 0\text{'s in } w \text{ is even}\}
\]
